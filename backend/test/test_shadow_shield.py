"""Tests for Shadow Shield ability.

Shadow Shield: when at full HP, reduces incoming damage by 50% (halves damage).

Test cases:
1. 0 SpA Bulbasaur Ice Beam vs. 0 HP / 0 SpD Shadow Shield Bulbasaur: 65-77 (28.1 - 33.3%)
   Possible damage amounts: (65, 66, 66, 67, 68, 69, 70, 70, 71, 72, 73, 73, 74, 75, 76, 77)

2. 0 Atk Bulbasaur Solar Blade vs. 0 HP / 0 Def Shadow Shield Bulbasaur: 16-20 (6.9 - 8.6%)
   Possible damage amounts: (16, 17, 17, 17, 17, 18, 18, 18, 18, 18, 18, 19, 19, 19, 19, 20)
"""
import sys
import json
...
def test_shadow_shield_ice_beam():
    """0 SpA Bulbasaur Ice Beam vs. Shadow Shield Bulbasaur."""
    ...
    print(f"Ice Beam vs Shadow Shield: {actual}")
    assert actual == expected, f"Expected {expected}, got {actual}"


def test_shadow_shield_solar_blade():
    """0 Atk Bulbasaur Solar Blade vs. Shadow Shield Bulbasaur."""
    attacker = get_pokemon_stats(
        species="bulbasaur",
        level=100,
        evs={"hp": 0, "attack": 0, "defense": 0, "special-attack": 0, "special-defense": 0, "speed": 0},
    ...
    move = {"name": "solar-blade", "power": 125, "type": "grass", "damage_class": "physical"}
    ...
    expected = (16, 17, 17, 17, 17, 18, 18, 18, 18, 18, 18, 19, 19, 19, 19, 20)
    actual = tuple(result["damage_all"])

    print(f"Solar Blade vs Shadow Shield: {actual}")
    assert actual == expected, f"Expected {expected}, got {actual}"


if __name__ == "__main__":
    test_shadow_shield_ice_beam()
    test_shadow_shield_solar_blade()
    print("All Shadow Shield tests passed!")
