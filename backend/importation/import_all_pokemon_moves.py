...
async def fetch_pokemon_moves(session: aiohttp.ClientSession, sem: asyncio.Semaphore, entry: dict):
    url = entry["url"]
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            return sort_id, pk_id_str, [m["move"]["name"] for m in data["moves"]]
        except Exception: await asyncio.sleep(BACKOFF_BASE * (2 ** attempt))
    return None

async def build(save_to="data/all_pokemon_moves.json"):
    ...
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=TIMEOUT_SECS), headers=HEADERS) as session:
        ...
        tasks = [fetch_pokemon_moves(session, sem, e) for e in entries]
    ...
    print(f"\n✓ Saved {len(final_dict)} pokemon move-lists → {save_to}")
